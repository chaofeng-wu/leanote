/*
 * @Author: Ethan Wu
 * @Date: 2021-07-03 11:45:46
 * @LastEditTime: 2021-07-03 20:48:00
 * @FilePath: /leanote/public/js/app/editor-drop-paste.js
 */
// 在toggle成pre或ace时
// 最后没有元素, 或者元素不是p, 则在最后插入之
Paste = {};
Paste.insertPIfNotExists = function() {
...
Paste.Process.prototype.update = function(process) {
    var me = this;
    // 98%, 不要小数
...
Paste.Process.prototype.replace = function(src) {
    var me = this;
    getImageSize(src, function() {
        $('#' + me.id).replaceWith('<img src="' + src + '" />');
    });
};
Paste.Process.prototype.remove = function() {
    var me = this;
    $('#' + me.id).remove();
};

// 当url改变时, 得到图片的大小
Paste.getImageSize = function(url, callback){
...
var i = 1;
Paste.insertImage = function(data) {
    ...
    if(fileId) {
        ...
        if(curNote && curNote.UserId != UserInfo.UserId) {
            (function(data) {
                Net.ajaxPost("/file/copyImage", {userId: UserInfo.UserId, fileId: fileId, toUserId: curNote.UserId}, function(re) {
                    if(reIsOk(re) && re.Id) {
...
// pasteImage
Paste.pasteImageInit =  function() {
    var curNote;
    // Initialize the jQuery File Upload plugin
    var dom, editor;
    // 2015/4/17 添加wmd-input markdown paste image
    $('#editorContent, #editorMD').fileupload({
        dataType: 'json',
        pasteZone: $('#editorContent, #editorMD'),
        dropZone: '', // 只允许paste
        maxFileSize: 210000,
        url: "/file/pasteImage",
        paramName: 'file',
        ...
        */
        ...
        add: function(e, data) {
            ...
            setTimeout(function () {
                ...
                if(!note.IsMarkdown) {
                    var process = new Paste.Process(editor);
                }
            ...
            if (data.result.Ok == true) {
                ...
                if(curNote && !curNote.IsMarkdown) {
                    ...
                    MarkdownEditor && MarkdownEditor.insertImage(src, title);
                }
